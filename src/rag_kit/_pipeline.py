...
class Pipeline:
    ...
    def __init__(
        ...
        # When True, chunk-derived TOC headings are generated by the cheap
        # router model (one batched call per query) instead of the
        # deterministic first-line heuristic — meaningful, structured
        # headings ("Relief valve sizing — orifice area formula") at a
        # small per-query cost. Default OFF keeps learning free.
        self._toc_ai_headings = toc_ai_headings
    ...
    def _record_chunk_learnings(self, file_id: int, chunks: list[dict]) -> int:
        ...
        # Normalize to (chunk_index, full_text) — prefer storage text over
        # trimmed/partial snippets handed in by callers.
    ...
    def _ai_heading_batch(self, items: list[tuple[int, str]]) -> dict[int, str]:
        """One cheap-model call generating structured headings for chunks.

        Returns {chunk_index: heading}. Raises on failure — caller falls
        back to the deterministic heuristic.
        """
        if not items:
            return {}
        # Keep the prompt small: up to 10 chunks, ~200 chars each.
        entries = []
        for ci, text in items[:10]:
            flat = re.sub(r"\s+", " ", text).strip()
            entries.append(f"[chunk {ci}] {flat[:200]}")
        prompt = (
            "You are building a book-style table of contents for a "
            "technical document. For each chunk excerpt below, write ONE "
            "short, meaningful heading (2-8 words) that names what the "
            "chunk is about — a topic label, not a sentence, not a "
            "question. Structure: 'Topic — subtopic' when the chunk has "
            "a clear part/whole relation; otherwise a plain noun phrase. "
            "Do NOT quote the excerpt verbatim.\n\n"
            "Excerpts:\n" + "\n\n".join(entries) + "\n\n"
            "Output ONLY valid JSON:\n"
